...
### Deletion lifecycle (tombstone-first, durable async purge)

Deletion of user data is high-impact, so it is fail-closed, tenant/Event-scoped, idempotent, and reversible only through the documented backups. It follows a tombstone-first design: the resource becomes non-searchable and non-downloadable immediately and synchronously, and the underlying data in PostgreSQL, MinIO, and Qdrant is purged durably and asynchronously.

- **Immediate tombstone (synchronous, in one transaction).** Photo delete sets the photo to the terminal `deleted` state; Event delete is the existing `archived` terminal state (there is no un-archive). The same transaction (a) neutralizes any not-yet-published `photo.processing.requested` outbox message for the affected photos so no new indexing work is enqueued, and (b) writes a versioned purge message to the transactional outbox (`photo.deletion.requested` with the object key, or `event.deletion.requested`) with a deterministic idempotency key (`photo.delete:{id}` / `event.delete:{id}`) under `ON CONFLICT DO NOTHING`. Because the outbox row commits atomically with the tombstone, a crash cannot lose the purge.
- **Immediate non-searchability and non-downloadability (DB-state enforcement).** Downloads already require `photos.status = 'ready'` and an active Event, so a tombstoned Photo or archived Event is instantly non-downloadable. Public search resolves its scope from the token (archived Events fail scope resolution) and, additionally, filters every candidate photo through an authoritative DB visibility check (`status = 'ready'` and active Event) before returning results. This guarantees a deleted Photo disappears from search the moment it is tombstoned, independent of any vector-index cleanup lag — the vector store is never the enforcement point.
- **Durable async purge (photo worker).** The worker consumes the purge messages and removes the underlying data everywhere it lives, always scoped by tenant and resource: MinIO objects under the resource key prefix (original + every derivative), Qdrant face vectors filtered by `organization_id` + `photo_id`/`event_id`, and PostgreSQL face rows (and, for an event, the photo tombstones). Steps run storage → vectors → database so a mid-way failure leaves the resource tombstoned and the message retryable rather than leaving orphaned, searchable data. Every step is idempotent, so a retried or duplicated purge causes no error and no double effect. Object-prefix removal refuses an empty or bucket-wide prefix so a purge can never wipe an unrelated scope.
- **Why async over synchronous cleanup.** Reusing the durable outbox and the worker (which already owns the Qdrant/MinIO/faces delete primitives) keeps deletion resilient to a MinIO/Qdrant/Face-worker outage: the tombstone still takes effect immediately, and the purge retries until it succeeds. A synchronous API-orchestrated purge would either block the request on slow multi-object deletes or silently leave orphans with no retry when a dependency is down.

### Retention

- **Selfie / biometric input:** never persisted. Public search runs ephemeral single-face inference, zeroes the selfie buffer before returning, and never writes selfie bytes or embeddings to storage, logs, or post-request state. There is nothing to retain or prune. (See Phase 6.)
- **Search-session records (`searches`):** short retention (default 90 days, `SEARCH_RETENTION_DAYS`). These are operational session/outcome rows and contain no biometric data. A row still referenced by a retained download record is kept until that download record is pruned, preserving audit linkage and foreign-key integrity.
- **Download decision records (`download_records`):** retained for the compliance window (default 365 days, `DOWNLOAD_RECORD_RETENTION_DAYS`). They already exclude signed URLs, object paths, tokens, and embeddings.
- **Authorization/audit records (`audit_records`):** retained the longest (default 365 days, `AUDIT_RETENTION_DAYS`); never pruned earlier than the documented window. Metadata excludes credentials, tokens, signed URLs, paths, image bytes, and biometric observations.
- **Operational rows:** terminal `photo_upload_sessions` (default 7 days) and delivered `outbox_messages` (default 7 days) are pruned as housekeeping.
- Pruning is enforced by an idempotent, env-driven, scheduled job (`scripts/retention-prune.sh`) that deletes only rows past their window and nothing else. It never deletes Photo, Event, faces, or object data — those follow the deletion lifecycle above.

### Backup and restore

PostgreSQL, MinIO, and Qdrant each have an env-driven backup with pinned tooling (matching the Compose image versions) and no committed secrets: `pg_dump`/`pg_restore` custom-format archives, `mc mirror` for the object bucket, and the Qdrant snapshot API. A restore drill (`scripts/backup/restore-drill.sh`) runs the full seed → backup → destroy → restore → verify cycle on a disposable Compose stack. Procedures live in `docs/backup-restore.md`.
