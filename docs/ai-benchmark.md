...
- Optional condition slicing uses manifest-declared opaque categorical dimension/value codes on query entries only. Do not encode names, locations, free-form notes, or other identifying or quasi-identifying data in these codes. Condition declarations, the fixed minimum slice size, and assignments are fingerprint-bound.
- Every declared condition value is reported deterministically. A slice exposes aggregate threshold points only when its query count reaches `minimum_slice_size`; smaller and zero-count slices are marked `suppressed` with no outcome metrics. Available slices reuse the global metric formulas and frozen thresholds, remain diagnostic only, and never choose a separate threshold. Suppression reduces disclosure from sparse aggregates but does not replace consent, access control, or dataset governance.
